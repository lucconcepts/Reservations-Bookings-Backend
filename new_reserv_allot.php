...
var mylink = "searchunitnr.php?t=" + rid;
  var items="";
...
$(function() {
$("#skills").autocomplete({
    source: 'searchallot.php',
    select: function(event, ui) { 
        $("#resortid").val(ui.item.id);
		 $("#taxrate").val(ui.item.tx);
		 $("#netrate").val(ui.item.netrate);
		 $("#from").val(ui.item.checkindate);
		 $("#to").val(ui.item.checkoutdate);
		 $("#balduedate").val(ui.item.checkoutdate);
		 $("#nights	").val(ui.item.nights);
...
VALUES ("'.$title.'" , "'.$fname.'" , "'.$lname.'" , "'.$phone.'" , "'.$phone2.'" , "'.$email.'" , "'.$email2.'" , "'.$address.'" , "'.$address2.'" , "'.$city.'" , "'.$state.'" , "'.$zip.'" , "'.$country.'" , "'.$custnotes.'" , "'.$bookdate.'" , "'.$agent.'" , "'.$source.'" , "'.$resort.'" , "'.$unittype.'" , "'.$invtype.'" , "'.$woname.'" , "'.$checkin3.'" , "'.$checkout3.'" , "'.$nights.'" , "'.$dailyrate.'" , "'.$nettotal.'" , "'.$totalrate.'" , "'.$netrate.'" , "'.$netrrateprofit.'" , "'.$paytype.'" , "'.$deposit.'" , "'.$balance.'" , "'.$cctype.'" , "'.$ccnum.'" , "'.$expdate.'" , "'.$balto.'" , "'.$balduedate2.'" , "'.$status.'" , "'.$supresvnum.'" , "'.$guestreq.'" , "'.$resvnote.'" , "'.$alotid.'" , "'.$iv.'");';
...
//Update Allotment that has been booked - change Available to NO	
$sqlalot = 'UPDATE allotment SET Available = "NO" WHERE ic = '.$alotid.'';
mysqli_query($connect,$sqlalot);
mysqli_close($connect);
?><script>
window.location = '/email_cust_allot.php'
</script>
<?
...
include 'rriconfigs/config.php';
$alotid = $_GET['alotid'];
$query = ('SELECT * FROM allotment LEFT JOIN resorts ON allotment.ResortId = resorts.id LEFT JOIN units on allotment.UnitId = units.id WHERE allotment.ic = '.$alotid.' ');  
...
<p><label class="field" for="source">Source Of Business:</label>
      ...
      <p><label class="field">Last Name:</label>
        ...
        <p><label class="field">Second Phone Number:</label>
        <input type="text" name="phone2" size="20" value=""> </p>
    <p><label class="field">Email Address:</label>
        <input type="text" name="email" size="25" value=""> </p>
        <p><label class="field">Second Email Address:</label>
...
<option value="Alabama">Alabama</option>
    ...
    <input type="text" name="resort" readonly="readonly" value="<?php echo $row['ResortName'];?>"/>
      
      <input type="text" name="resortid" hidden="" value="<?php echo $row['ResortId'];?>"/>
      </p>
      
      
      <input type="text" name="invtype" hidden="" value="Allot" required="required">
      <input type="text" name="alotid" hidden="" value="<?php echo $alotid; ?>" required="required">
      <input type="text" name="unittype" hidden="" value="<?php echo $row['UnitId']; ?>" required="required">

      <p><label class="field">Check In Date:</label>
      <input type="text" name="checkin" required="required" readonly="readonly" value="<?php echo date('m/d/Y', strtotime( $row['CheckInDate'] ));?> "/>
      </p>
      <p><label class="field">Check Out Date:</label>
      <input type="text" name="checkout" required="required" readonly="readonly" value="<?php echo date('m/d/Y', strtotime( $row['CheckOutDate'] ));?>" />
      </p>
      <p>
      
      <input type="text" name="nights" hidden="hidden" required="required" value="<?php echo $row['Nights'];?>" id="nights" />
      </p>
      <p>
      <label class="field">Tax Rate:</label>
      <input type="text" name="taxrate" readonly="readonly" required="required" id="taxrate" value="<?php echo $row['Taxrate'];?>" />
      </p>
